...
use gtk::{
  gio, glib, pango, Align, CheckButton, CustomFilter, Dialog, DialogFlags, Entry, FilterListModel, Label, ListBoxRow, NoSelection, ResponseType, SelectionMode,
};
...
impl Window {
  ...
  fn filter(&self) -> Option<CustomFilter> {
    ...
    let filter_open = CustomFilter::new(|obj| {
      ...
      let task_object = obj.downcast_ref::<TaskObject>().expect("The object needs to be of type `TaskObject`.");
  ...
  fn setup_collections(&self) {
    ...
    self.imp().collections.set(collections.clone()).expect("Could not set collections");
  ...
  fn restore_data(&self) {
    if let Ok(file) = File::open(data_path()) {
      ...
      let backup_data: Vec<CollectionData> = serde_json::from_reader(file).expect("It should be possible to read `backup_data` from the json file.");

      // Convert `Vec<CollectionData>` to `Vec<CollectionObject>`
      let collections: Vec<CollectionObject> = backup_data.into_iter().map(CollectionObject::from_collection_data).collect();
  ...
  fn set_current_collection(&self, collection: CollectionObject) {
    ...
    let tasks_changed_handler_id = tasks.connect_items_changed(clone!(@weak self as window => move |tasks, _, _, _| {
        window.set_task_list_visible(tasks);
    }));
    self.imp().tasks_changed_handler_id.replace(Some(tasks_changed_handler_id));
  ...
  fn create_task_row(task_object: &TaskObject) -> ActionRow {
    ...
    // Create row
    let row = ActionRow::builder().activatable_widget(&check_button).build();
    ...
    task_object
      ...
      .build();
    task_object.bind_property("content", &row, "title").flags(BindingFlags::SYNC_CREATE).build();
  ...
  fn setup_callbacks(&self) {
    ...
    self.imp().collections_list.connect_row_activated(clone!(@weak self as window => move |_, row| {
        let index = row.index();
  ...
  fn new_collection(&self) {
    ...
    dialog.connect_response(clone!(@weak self as window, @weak entry => move |dialog, response| {
        // Destroy dialog
        dialog.destroy();
